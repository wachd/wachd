...
func (db *DB) GetTeamConfig(ctx context.Context, teamID uuid.UUID) (*TeamConfig, error) {
	query := `
		SELECT
			...
			grafana_mcp_url, grafana_mcp_token_encrypted,
...
func (db *DB) UpsertTeamConfig(ctx context.Context, tc *TeamConfig) error {
	query := `
		...
		ON CONFLICT (team_id) DO UPDATE SET
			...
			grafana_mcp_url           = EXCLUDED.grafana_mcp_url,
			grafana_mcp_token_encrypted = EXCLUDED.grafana_mcp_token_encrypted,
