    ...
    <div className="space-y-4">
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
        <h2 className="text-base font-semibold text-blue-900 mb-1">
          No incidents yet &mdash; go back to the demo control panel to fire a scenario
        </h2>
        <p className="text-sm text-blue-700 mb-4">
          Click one of the 4 pre-built scenarios on the demo page to generate a realistic alert
          and watch Wachd produce a Claude AI root cause analysis in ~30 seconds.
        </p>
        <a
          href="https://demo.wachd.io"
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-8 text-center text-gray-400 text-sm">
        Incidents will appear here once a scenario fires.
      </div>
    </div>
  );
}
...
function SelfHostedEmptyState() {
  return (
    <div className="space-y-4">
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
        <h2 className="text-base font-semibold text-blue-900 mb-1">
          No incidents yet &mdash; here&apos;s how to get your first alert
        </h2>
        ...
        <ol className="space-y-3 text-sm text-blue-800">
          <li className="flex items-start gap-2">
            ...
            <span>
              <strong>Configure notifications</strong> &mdash; go to{' '}
          ...
          <li className="flex items-start gap-2">
            ...
            <span>
              <strong>Set up your on-call schedule</strong> &mdash; go to{' '}
          ...
          <li className="flex items-start gap-2">
            ...
            <span>
              <strong>Point your monitoring tool at the webhook</strong> &mdash; find your webhook URL in{' '}
              <Link href="/settings" className="underline hover:text-blue-600">Settings &rarr; Integrations</Link> and add it to Grafana, Datadog, or Prometheus.
            </span>
          </li>
          <li className="flex items-start gap-2">
            <span className="font-bold mt-0.5">4.</span>
            <span>
              <strong>Set your personal notification rules</strong> &mdash; go to{' '}
...
export function IncidentsView({ demoMode }: { demoMode: boolean }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {incidents.length === 0 ? (
        demoMode ? <DemoEmptyState /> : <SelfHostedEmptyState />
      ) : (
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className="text-sm text-gray-500">{formatDate(incident.fired_at)}</span>
                      </div>
                      <h3 className="text-lg font-medium text-gray-900 mb-1">{incident.title}</h3>
                      {incident.message && (
                        <p className="text-sm text-gray-600 line-clamp-2">{incident.message}</p>
                      )}
